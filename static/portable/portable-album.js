 ...
 */
/* portable-album.js — fork of album-viewer-init.js */
var albumVideoObserver = null;

function initPortableAlbum() {
  ...
  if (typeof FlairyData !== "undefined" && FlairyData.projectId) {
    projectId = String(FlairyData.projectId).trim();
  } else if (projectId) {
  ...
  var pageFlipAudio = { play: function () {} };
  function portableNormalizePath(path) {
    if (!path) return "";
    return String(path).replace(/\\/g, "/").trim();
  }
  function portableLookup(map, path) {
    if (!map || !path) return "";
    var p = portableNormalizePath(path);
    if (map[p]) return map[p];
    var fn = p.split("/").pop();
    for (var k in map) {
      if (k.split("/").pop() === fn) return map[k];
    }
    return "";
  }
  function portableResolveImage(path) {
    return portableLookup(typeof FlairyData !== "undefined" ? FlairyData.assets : null, path);
  }
  function portableResolveStream(path) {
    return portableLookup(typeof FlairyData !== "undefined" ? FlairyData.streams : null, path);
  }


  function toRawUrl(path) {
    if (!path) return "";
    var stream = portableResolveStream(path);
    if (stream) return stream;
    var embedded = portableResolveImage(path);
  ...
  function toAlbumImageUrlFromStoragePath(path, opts) {
    if (!path || isVideoPath(path)) return toRawUrl(path);
    var embedded = portableResolveImage(path);
    if (embedded) return embedded;
    return toRawUrl(path);
  }

  function albumImageAttrsString(path, opts) {
    opts = opts || {};
    if (!path || isVideoPath(path)) return { src: toRawUrl(path) };
    var embedded = portableResolveImage(path);
    if (embedded) return { src: embedded };
    return { src: toRawUrl(path) };
  }
  ...
  function bootstrapPortableLayout(data) {
      console.log("[Portable] layout pages=" + (data.pages ? data.pages.length : 0));
      layout = data;
      ...
      console.log("[Portable] Init complete");
    }

  try {
    if (typeof FlairyData === "undefined" || !FlairyData.layout) throw new Error("FlairyData missing");
    bootstrapPortableLayout(FlairyData.layout);
    var titleEl = document.querySelector(".album-viewer-title");
    if (titleEl && FlairyData.title) titleEl.textContent = FlairyData.title;
  } catch (err) {
    var loadingStateEl = document.getElementById("loadingState");
    if (loadingStateEl) loadingStateEl.classList.add("hidden");
    var errorStateEl = document.getElementById("errorState");
    ...
    console.warn("Portable layout load failed:", err);
  }
...
document.addEventListener('DOMContentLoaded', function () {
  if (document.getElementById('flipbookRoot')) initPortableAlbum();
});
